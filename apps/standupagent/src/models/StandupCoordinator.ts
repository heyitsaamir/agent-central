...
export class StandupCoordinator {
    ...
    async setDefaultStandup(userId: string, tenantId: string, standupGroupIdOrName: string): Promise<Result<{ message: string }>> {
        return await this.userService.setDefaultStandup(userId, tenantId, standupGroupIdOrName);
    }



    async getStandupsForUser(userId: string, tenantId: string): Promise<Result<{ standups: Array<{ conversationId: string; isDefault: boolean; conversationName: string | null }> }>> {
    ...
    async getWorkItemsFromDefaultGroup(userId: string, tenantId: string): Promise<Result<{ workItems: string[]; groupId: string; groupName: string | null }>> {
        try {
            // Get user's settings and standup groups
            const userStandupsResult = await this.userService.getStandupsForUser(userId, tenantId);
            if (userStandupsResult.type === "error") {
            ...
            const workItems = userResponse?.completedWork
                ? userResponse.completedWork.split('\n').filter(item => item.trim())
                : [];
            ...
            return {
                ...
                data: {
                    ...
                    groupName: group.conversationName
                },
